...
interface ProductCtaProps {
  ...
  primaryButtonColor: ColorType;
  secondaryButtonColor?: ColorType;
}

function getButtonClasses(color: ColorType) {
  const colorMap = {
...
export function ProductCta({ title, subtitle, primaryCta, secondaryCta, primaryButtonColor }: ProductCtaProps) {
  ...
  const primaryBtnClasses = getButtonClasses(primaryButtonColor);
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 items-center gap-8 px-8 py-4 md:px-12 md:py-6 lg:px-16 lg:py-8">
            ...
            >
              ...
              <motion.div className="flex flex-col w-full sm:w-auto gap-4" variants={itemVariants}>
                <Button asChild size="lg" className={`h-12 w-full sm:w-auto shadow-md ${primaryBtnClasses}`}>
