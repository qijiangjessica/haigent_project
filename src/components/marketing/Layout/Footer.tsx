      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6">
        <div className="pt-12 pb-8">
          <Link href="/" className="rounded-md p-2">
            <Image src="/Logo_simple_black.png" alt="Haigent" width={100} height={100} />
          </Link>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-8 py-8">
          {footerLinks.map((item, i) => (
            <div key={i}>
              <h3 className="mb-4 text-xs font-semibold text-white uppercase tracking-wider">{item.title}</h3>
          ...
          <div className="flex gap-2 items-center">
            {socialLinks.map(({ svg, href, label }, i) => (
              <Link
                ...
                aria-label={label}
                className={buttonVariants({ variant: "outline", size: "icon" }) + " border-border bg-background hover:bg-primary hover:border-primary/50 text-foreground/70 hover:text-white transition-colors"}
                key={i}
              >
                {svg}
              </Link>
